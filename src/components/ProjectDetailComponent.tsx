        ...
        return section.parts.map((partGroup, groupIndex) => (
            <div key={groupIndex} className="mb-4">
                {partGroup.map((part, partIndex) => {
                    if (part.type === 'text') {
                        return <p key={`${groupIndex}-${partIndex}`} className="text-lg text-slate-700 leading-relaxed whitespace-pre-wrap mb-4">{part.content}</p>;
                    }
                    if (part.type === 'image') {
    ...
    return (
        <main className="max-w-5xl mx-auto p-4 sm:p-8 bg-white text-slate-800">
            ...
            <section className="mb-16">
                ...
                <p className="text-lg text-slate-700 leading-relaxed whitespace-pre-wrap">{project.description}</p>
            </section>
            <hr className="my-12 border-t-2 border-slate-200" />
