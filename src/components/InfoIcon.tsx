...
export const InfoIcon: React.FC<InfoIconProps> = ({
  ...
  return (
    <a
      ...
      className="flex items-center p-4 rounded-lg hover:bg-surface-muted transition-colors duration-300"
    >
      <div className="w-8 h-8 mr-4 flex-shrink-0">
        {children}
      </div>
      <div className="min-w-0">
        <p className="font-semibold text-content-strong">{capitalizeFirst(activeKeyType)}</p>
      </div>
    </a>
  );
};
